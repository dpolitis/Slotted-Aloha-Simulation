...
#define M 100                             // number of stations
#define N 50                              // number of channels
#define FB 3                              // receiving buffer length 
#define R 100000000                       // number of time-slots per invocation

/* global variable definition */
int i, ii, j, jj;                         // counters of transmissions per invocation
int k, f, g;                              // general purpose counters
...
int sim[M+1][7];                          // simulation array
...
int main () {
  ...
  do {
    ...
    p+= 0.001;                            // increase probability at each invocation
    B = 0;
    C = 0;
    S = 0;
    busy = 0;

    /* until simulation cycle is over */
    do {
      ...
      /* fill simulation matrix (k is arbitrary source): channel, boolean busy, destination,
      boolean success, total packet counter on destination, successful packet counter on 
      destination (initially all packets are considered successful), boolean destruction on channels */
      for (k = 0; k < i + j; k++) {
      ...
      for (k = 0; k < i + j; k++) {
        for (f = k + 1; f < i + j; f++) {
          /* find duplicates (packets on same channel) */
          if (sim[k][0] == sim[f][0]) {
            /* mark as failed, because of same channel use */
            sim[k][3] = 0;
            sim[k][6] = 1;
            sim[f][3] = 0;
            sim[f][6] = 1;
      ...
      for (k = 0; k < M; k++) {
        /* check for same destination k */
        for (f = 0; f < i + j; f++) {
          if ((sim[f][2] == k) && (sim[f][6] != 1)) {
          /* increase receiving buffer packet counter on all instances */
            for (g = 0; g < i + j; g++) {
              if ((sim[g][2] == k) && (sim[g][3] != 0)) { sim[g][4]++; }
            }
          }
        }
      }
      ...
      for (k = 0; k < i + j; k++) {
        /* mark as failed, because of receiving buffer overflow, if not failed already */
        if ((sim[k][4] > FB) && (sim[k][6] != 1)) {
          /* throw dice to say if package accepted */
          if (sim[k][5] >= FB) { sim[k][3] = 0; }
          else { sim[k][3] = genrand64_int1(1); }
        }
        /* package accepted */
        if (sim[k][3] == 1) {
          for (g = 0; g < i + j; g++) {
            if (sim[g][2] == sim[k][2]) { sim[g][5]++; }
          }
        }
      }
      ...
      for (k = 0; k < i + j; k++) {
        if ((sim[k][3] == 0) && (sim[k][6] != 1) && (sim[k][4] > FB) && (sim[k][5] < FB)) {
          /* accept package that was randomly rejected at previous step */
          sim[k][3] = 1;
          for (g = 0; g < i + j; g++) {
            if (sim[g][2] == sim[k][2]) { sim[g][5]++; }
          }
    ...
    /* printout simulated throughput, calculated delay, mean busy stations */
